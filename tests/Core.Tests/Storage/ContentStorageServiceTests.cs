...
/// </summary>
internal sealed class ContentStorageServiceTests : IDisposable
{
    ...
    public async Task UpsertAsync_WithEmptyId_GeneratesNewIdAsync()
    {
        ...
        // Act
        var resultId = await _service.UpsertAsync(request).ConfigureAwait(false).ConfigureAwait(false);

        // Assert
        Assert.Equal("test_id_00001", resultId); // First generated ID

        // Verify content was created
        var content = await _service.GetByIdAsync(resultId).ConfigureAwait(false).ConfigureAwait(false);
    ...
    public async Task UpsertAsync_ReplacesExistingContentAsync()
    {
        ...
        await _service.UpsertAsync(initialRequest).ConfigureAwait(false).ConfigureAwait(false);
        ...
        await _service.UpsertAsync(replaceRequest).ConfigureAwait(false).ConfigureAwait(false);

        // Wait for processing to complete
        await Task.Delay(100).ConfigureAwait(false).ConfigureAwait(false);

        // Assert
        var content = await _service.GetByIdAsync("test_id_replace").ConfigureAwait(false).ConfigureAwait(false);
        Assert.NotNull(content);
    ...
    public async Task UpsertAsync_StoresTagsAsync()
    {
        ...
        // Act
        var resultId = await _service.UpsertAsync(request).ConfigureAwait(false).ConfigureAwait(false);
        await Task.Delay(100).ConfigureAwait(false).ConfigureAwait(false); // Wait for processing
    ...
    public async Task DeleteAsync_RemovesExistingContentAsync()
    {
        ...
        await _service.UpsertAsync(request).ConfigureAwait(false).ConfigureAwait(false);
        await Task.Delay(100).ConfigureAwait(false).ConfigureAwait(false); // Wait for processing

        // Verify content exists
        var contentBefore = await _service.GetByIdAsync("test_id_delete").ConfigureAwait(false).ConfigureAwait(false);
        Assert.NotNull(contentBefore);

        // Act - Delete the content
        await _service.DeleteAsync("test_id_delete").ConfigureAwait(false).ConfigureAwait(false);
        await Task.Delay(100).ConfigureAwait(false).ConfigureAwait(false); // Wait for processing

        // Assert - Content should be gone
        var contentAfter = await _service.GetByIdAsync("test_id_delete").ConfigureAwait(false).ConfigureAwait(false);
    ...
    public async Task DeleteAsync_IsIdempotentAsync()
    {
        ...
        await _service.DeleteAsync("non_existent_id").ConfigureAwait(false).ConfigureAwait(false);
        await Task.Delay(100).ConfigureAwait(false).ConfigureAwait(false); // Wait for processing

        // Assert - No exception thrown, verify content doesn't exist
        var content = await _service.GetByIdAsync("non_existent_id").ConfigureAwait(false).ConfigureAwait(false);
        Assert.Null(content);
    }
    ...
    public async Task CountAsync_ReturnsCorrectCountAsync()
    {
        ...
        await Task.Delay(500).ConfigureAwait(false).ConfigureAwait(false); // Wait for all to process

        // Act
        var count = await _service.CountAsync().ConfigureAwait(false).ConfigureAwait(false);
    ...
    public async Task UpsertAsync_QueuesOperationSuccessfullyAsync()
    {
        ...
        var operation = await _context.Operations
            .FirstOrDefaultAsync(o => o.ContentId == resultId).ConfigureAwait(false).ConfigureAwait(false);
    ...
    public async Task DeleteAsync_QueuesOperationSuccessfullyAsync()
    {
        ...
        // Act
        await _service.DeleteAsync(contentId).ConfigureAwait(false).ConfigureAwait(false);

        // Assert - Operation should be queued
        var operation = await _context.Operations
            .FirstOrDefaultAsync(o => o.ContentId == contentId).ConfigureAwait(false).ConfigureAwait(false);
    ...
    public async Task ConcurrentUpserts_LastOneWinsAsync()
    {
        ...
        await Task.WhenAll(task1, task2, task3).ConfigureAwait(false).ConfigureAwait(false);
        await Task.Delay(300).ConfigureAwait(false).ConfigureAwait(false); // Wait for all operations to process

        // Assert - Last version should win
        var content = await _service.GetByIdAsync(contentId).ConfigureAwait(false).ConfigureAwait(false);
        Assert.NotNull(content);
    ...
    public async Task OperationCancellation_SupersededUpsertsAsync()
    {
        ...
        await Task.Delay(500).ConfigureAwait(false).ConfigureAwait(false); // Wait for processing
    ...
    public async Task UpsertAsync_HandlesLargeContentAsync()
    {
        ...
        // Act
        var resultId = await _service.UpsertAsync(request).ConfigureAwait(false).ConfigureAwait(false);
        await Task.Delay(1000).ConfigureAwait(false).ConfigureAwait(false); // Wait longer for large content processing
