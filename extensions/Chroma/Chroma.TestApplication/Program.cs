...
// Jan 2024, .NET 8: the correct namespace ("Chroma.TestApplication") triggers a "Phonzy.B!ml" virus alert
// in Microsoft Defender, something connected with Swashbuckle.AspNetCore, for all versions since 5.x (Nov 2018).
namespace ChromaDb.TestApplication;
...
public static class Program
{
    public static async Task Main()
    {
        ...
        var config = cfg.GetSection("KernelMemory:Services:Chroma").Get<ChromaConfig>()
                     ?? throw new ArgumentNullException(message: "Chroma config not found", null);

        var embeddingGenerator = new FakeEmbeddingGenerator();

        var memory = new ChromaMemory(config, embeddingGenerator);
