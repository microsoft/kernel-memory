...
public sealed class MsWordDecoder : IContentDecoder
{
    ...
    public Task<FileContent> DecodeAsync(Stream data, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (paragraphs != null)
            {
                foreach (Paragraph p in paragraphs)
                {
                    ...
                    if (lastRenderedPageBreak != null)
                    {
                        ...
                        sb.Clear();
                        result.Sections.Add(new Chunk(pageNumber, pageContent, Chunk.Meta(sentencesAreComplete: true)));
                        pageNumber++;
                    }
            ...
            result.Sections.Add(new Chunk(pageNumber, lastPageContent, Chunk.Meta(sentencesAreComplete: true)));
